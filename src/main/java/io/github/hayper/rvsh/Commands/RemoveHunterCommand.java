...
public class RemoveHunterCommand implements CommandExecutor {
    ...
    public RemoveHunterCommand(Main pl) {
        this.pl = pl;
    }
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (successfulPlayers.size() == 0) {
            ...
            sender.sendMessage(String.format("Failed to remove %s from hunter team", errorText.toString()));
        } else {
            sender.sendMessage(String.format("Removed %s from hunter team", successfulText.toString()));
        }
        return true;
    }
