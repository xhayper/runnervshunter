...
public class RemoveRunnerCommand implements CommandExecutor {

    Main pl;

    public RemoveRunnerCommand(Main pl) {
        this.pl = pl;
    }
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        for (String playerName : args) {
            ...
            if (targetPlayer != null) {
                if (GameManager.runnerList.contains(targetPlayer)) {
        ...
        if (successfulPlayers.size() == 0) {
            ...
            sender.sendMessage(String.format("Failed to remove %s from runner team", errorText.toString()));
        } else {
            sender.sendMessage(String.format("Removed %s from runner team", successfulText.toString()));
        }
        return true;
    }
