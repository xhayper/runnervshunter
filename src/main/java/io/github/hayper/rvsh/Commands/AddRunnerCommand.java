...
public class AddRunnerCommand implements CommandExecutor {
    ...
    public AddRunnerCommand(Main pl) {
        this.pl = pl;
    }
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (successfulPlayers.size() == 0) {
            ...
            sender.sendMessage(String.format("Failed to add %s to runner team", errorText.toString()));
        } else {
            sender.sendMessage(String.format("Added %s to runner team", successfulText.toString()));
        }
        return true;
    }
